...
static int32_t atPowerOn(uGnssPrivateInstance_t *pInstance, uAtClientHandle_t atHandle, bool ugindOnNotOff) {
  int32_t errorCode;
  uint64_t y;
  ...
  if (y != 1) {
    ...
    if (errorCode == 0) {
      ...
      for (size_t x = 0; (errorCode < 0) && (x < U_GNSS_AT_POWER_ON_RETRIES + 1); x++) {
  ...
  if (gUGnssPrivateMutex != NULL) {
    ...
    if (pInstance != NULL) {
      ...
      if (pInstance->pinGnssEnablePower >= 0) {
        errorCode = (int32_t) U_ERROR_COMMON_PLATFORM;
        if (uPortGpioSet(pInstance->pinGnssEnablePower, pInstance->pinGnssEnablePowerOnState) == 0) {
        ...
        if (pInstance->transportType == U_GNSS_TRANSPORT_AT) {
          ...
          if (atHandle != NULL) {
            ...
            if (pInstance->portNumber != 3) {
              ...
              if (uGnssPrivateSendOnlyCheckStreamUbxMessage(pInstance, 0x06, 0x04, message, 4) > 0) {
                errorCode = (int32_t) U_ERROR_COMMON_SUCCESS;
                if (pInstance->pModule->moduleType == U_GNSS_MODULE_TYPE_M8) {
                  ...
                  if (uGnssPrivateSendReceiveUbxMessage(pInstance, 0x0a, 0x28, NULL, 0, message, 8) == 8) {
                    errorCode = (int32_t) U_ERROR_COMMON_SUCCESS;
                    ...
                    if (message[3] & 0x08) {
                      ...
                      errorCode = uGnssPrivateSendReceiveUbxMessage(pInstance, 0x06, 0x04, message, 4, NULL, 0);
                      if (errorCode == 0) {
      ...
      if ((errorCode < 0) && (pInstance->pinGnssEnablePower >= 0)) {
        ...
        uPortGpioSet(pInstance->pinGnssEnablePower, (int32_t) !pInstance->pinGnssEnablePowerOnState);
      }
    }
...
bool uGnssPwrIsAlive(uDeviceHandle_t gnssHandle) {
  ...
  if (gUGnssPrivateMutex != NULL) {
    ...
    if (pInstance != NULL) {
      ...
      if (uGnssPrivateSendReceiveUbxMessage(pInstance, 0x0a, 0x04, NULL, 0, buffer, sizeof(buffer)) > 0) {
        isAlive = true;
      }
    ...
    if (pInstance != NULL) {
      if (pInstance->transportType == U_GNSS_TRANSPORT_AT) {
        ...
        if (atHandle != NULL) {
          ...
          if (uGnssPrivateSendOnlyCheckStreamUbxMessage(pInstance, 0x06, 0x04, message, sizeof(message)) > 0) {
            errorCode = (int32_t) U_ERROR_COMMON_SUCCESS;
          }
        }
      }
      ...
      if (pInstance->pinGnssEnablePower >= 0) {
        ...
        errorCode = uPortGpioSet(pInstance->pinGnssEnablePower, (int32_t) !pInstance->pinGnssEnablePowerOnState);
      }
    }
    ...
    if (pInstance != NULL) {
      ...
      if (pInstance->transportType != U_GNSS_TRANSPORT_AT) {
        ...
        if (pInstance->pModule->moduleType != U_GNSS_MODULE_TYPE_M7) {
          ...
          //lint -restore
          errorCode = uGnssPrivateSendReceiveUbxMessage(pInstance, 0x02, 0x41, message, sizeof(message), NULL, 0);
        } else {
          char message[8] = {
              0 };
          message[4] = 0x02;  // Backup
          //lint -save -e569 Suppress loss of information: OK on all our compilers
          errorCode = uGnssPrivateSendReceiveUbxMessage(pInstance, 0x02, 0x41, message, sizeof(message), NULL, 0);
        }
        if ((errorCode == 0) && (pInstance->pinGnssEnablePower >= 0)) {
