...
static const char *const gpTransportTypeText[] = {
    "None",       // U_GNSS_TRANSPORT_NONE
...
//lint -esym(1746, transportHandle) Suppress could
// be made const: it is!
int32_t uGnssAdd(uGnssModuleType_t moduleType, uGnssTransportType_t transportType, const uGnssTransportHandle_t transportHandle,
  ...
  int32_t pinGnssEnablePowerOnState =
      (pinGnssEnablePower & U_GNSS_PIN_INVERTED) ? !U_GNSS_PIN_ENABLE_POWER_ON_STATE : U_GNSS_PIN_ENABLE_POWER_ON_STATE;
  ...
  if (gUGnssPrivateMutex != NULL) {
    ...
    if (pDevInstance != NULL) {
      ...
      if (((size_t) moduleType < gUGnssPrivateModuleListSize)
          && ((transportType > U_GNSS_TRANSPORT_NONE) && (transportType < U_GNSS_TRANSPORT_MAX_NUM))
          && ((transportType == U_GNSS_TRANSPORT_I2C) || (pGetGnssInstanceTransportHandle(transportType, transportHandle) == NULL))) {
        errorCode = (int32_t) U_ERROR_COMMON_NO_MEMORY;
        ...
        if (pInstance != NULL) {
          ...
          if (errorCode == 0) {
            ...
            if (pinGnssEnablePower >= 0) {
              uPortLog("%d (0x%02x), set to %d to power on GNSS", pinGnssEnablePower, pinGnssEnablePower,
                       pinGnssEnablePowerOnState);
              if (leavePowerAlone) {
            ...
            if (pinGnssEnablePower >= 0) {
              if (!leavePowerAlone) {
                ...
                platformError = uPortGpioSet(pinGnssEnablePower, (int32_t) !pinGnssEnablePowerOnState);
              }
              if (platformError == 0) {
            ...
            if (pInstance->transportType != U_GNSS_TRANSPORT_AT) {
              ...
              if (pInstance->pLinearBuffer != NULL) {
                ...
                if (pInstance->pTemporaryBuffer != NULL) {
                  ...
                  errorCode = uRingBufferCreateWithReadHandle(&(pInstance->ringBuffer), pInstance->pLinearBuffer,
                      ...
                      if (errorCode >= 0) {
                        ...
                        if (pInstance->transportType == U_GNSS_TRANSPORT_SPI) {
                          ...
                          if (pInstance->pSpiLinearBuffer != NULL) {
                            ...
                            if (pInstance->pSpiRingBuffer != NULL) {
                              errorCode = uRingBufferCreate(pInstance->pSpiRingBuffer, pInstance->pSpiLinearBuffer,
...
int32_t uGnssSetIntermediate(uDeviceHandle_t gnssHandle, uDeviceHandle_t intermediateHandle) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_NOT_INITIALISED;
...
int32_t uGnssGetIntermediate(uDeviceHandle_t gnssHandle, uDeviceHandle_t *pIntermediateHandle) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_NOT_INITIALISED;
  uGnssPrivateInstance_t *pInstance;
...
// given instance.
int32_t uGnssGetTransportHandle(uDeviceHandle_t gnssHandle, uGnssTransportType_t *pTransportType,
