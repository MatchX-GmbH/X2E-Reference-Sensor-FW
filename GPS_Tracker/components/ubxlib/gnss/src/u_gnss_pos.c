...
typedef struct {
  ...
  void (*pCallback)(uDeviceHandle_t gnssHandle, int32_t errorCode, int32_t latitudeX1e7, int32_t longitudeX1e7,
...
static int32_t posDecode(char *pMessage, int32_t *pLatitudeX1e7, int32_t *pLongitudeX1e7, int32_t *pAltitudeMillimetres,
                         int32_t *pRadiusMillimetres, int32_t *pSpeedMillimetresPerSecond, int32_t *pSvs, int64_t *pTimeUtc,
                         bool printIt) {
...
// Establish position.
static int32_t posGet(uGnssPrivateInstance_t *pInstance, int32_t *pLatitudeX1e7, int32_t *pLongitudeX1e7,
                      int32_t *pAltitudeMillimetres, int32_t *pRadiusMillimetres, int32_t *pSpeedMillimetresPerSecond,
                      int32_t *pSvs, int64_t *pTimeUtc, bool printIt) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_TIMEOUT;
  // Enough room for the body of the UBX-NAV-PVT message
  if (pInstance->pModule->moduleType == U_GNSS_MODULE_TYPE_M7) {
    char message_M7[84] = {
        0 };

    errorCode = uGnssPrivateSendReceiveUbxMessage(pInstance, 0x01, 0x07, NULL, 0, message_M7, sizeof(message_M7));
    if (errorCode == sizeof(message_M7)) {
      // Got the correct message body length, process it
      errorCode = posDecode(message_M7, pLatitudeX1e7, pLongitudeX1e7, pAltitudeMillimetres, pRadiusMillimetres,
                            pSpeedMillimetresPerSecond, pSvs, pTimeUtc, printIt);
    } else {
      if (errorCode >= 0) {
    ...
    errorCode = uGnssPrivateSendReceiveUbxMessage(pInstance, 0x01, 0x07, NULL, 0, message, sizeof(message));
    if (errorCode == sizeof(message)) {
      // Got the correct message body length, process it
      errorCode = posDecode(message, pLatitudeX1e7, pLongitudeX1e7, pAltitudeMillimetres, pRadiusMillimetres,
                            pSpeedMillimetresPerSecond, pSvs, pTimeUtc, printIt);
    } else {
      if (errorCode >= 0) {
        errorCode = (int32_t) U_ERROR_COMMON_DEVICE_ERROR;
...
static void posGetTask(void *pParameter) {
  ...
  while ((taskParameters.pInstance->posTaskFlags & U_GNSS_POS_TASK_FLAG_KEEP_GOING)
      && (errorCode == (int32_t) U_ERROR_COMMON_TIMEOUT) && ((uPortGetTickTimeMs() - startTime) / 1000 < U_GNSS_POS_TIMEOUT_SECONDS)) {
    // Call posGet() to do the work
    errorCode = posGet(taskParameters.pInstance, &latitudeX1e7, &longitudeX1e7, &altitudeMillimetres, &radiusMillimetres,
                       &speedMillimetresPerSecond, &svs, &timeUtc, false);
    if (errorCode != 0) {
  ...
  // Call the callback
  taskParameters.pCallback(taskParameters.gnssHandle, errorCode, latitudeX1e7, longitudeX1e7, altitudeMillimetres,
                           radiusMillimetres, speedMillimetresPerSecond, svs, timeUtc);
...
static void messageCallback(uDeviceHandle_t gnssHandle, const uGnssMessageId_t *pMessageId, int32_t errorCodeOrLength,
                            void *pCallbackParam) {
  uGnssPrivateInstance_t *pInstance = (uGnssPrivateInstance_t*) pCallbackParam;
  char message[92 + U_UBX_PROTOCOL_OVERHEAD_LENGTH_BYTES] = {
      0 };
  ...
  if (errorCodeOrLength > 0) {
    ...
    uGnssMsgReceiveCallbackRead(gnssHandle, message, errorCodeOrLength);
    // Decode the body
    errorCodeOrLength = posDecode(message + U_UBX_PROTOCOL_HEADER_LENGTH_BYTES, &latitudeX1e7, &longitudeX1e7, &altitudeMillimetres,
                                  &radiusMillimetres, &speedMillimetresPerSecond, &svs, &timeUtc, true);
    if (errorCodeOrLength == 0) {
      // Call the callback
      ...
      pInstance->pStreamedPosition->pCallback(pInstance->pStreamedPosition->gnssHandle, errorCodeOrLength, latitudeX1e7,
                                              longitudeX1e7, altitudeMillimetres, radiusMillimetres, speedMillimetresPerSecond, svs,
                                              timeUtc);
    }
  }
}
...
int32_t uGnssPosGet(uDeviceHandle_t gnssHandle, int32_t *pLatitudeX1e7, int32_t *pLongitudeX1e7, int32_t *pAltitudeMillimetres,
                    int32_t *pRadiusMillimetres, int32_t *pSpeedMillimetresPerSecond, int32_t *pSvs, int64_t *pTimeUtc,
  ...
  if (gUGnssPrivateMutex != NULL) {
    ...
    if (pInstance != NULL) {
      ...
      while ((errorCode == (int32_t) U_ERROR_COMMON_TIMEOUT)
          && (((pKeepGoingCallback == NULL) && (uPortGetTickTimeMs() - startTime) / 1000 < U_GNSS_POS_TIMEOUT_SECONDS)
              || ((pKeepGoingCallback != NULL) && pKeepGoingCallback(gnssHandle)))) {
        // Call posGet() to do the work
        errorCode = posGet(pInstance, pLatitudeX1e7, pLongitudeX1e7, pAltitudeMillimetres, pRadiusMillimetres,
                           pSpeedMillimetresPerSecond, pSvs, pTimeUtc, true);
      }
    }
      ...
      if (pInstance->posTaskFlags == 0) {
        ...
        if (errorCode == 0) {
          ...
          if (pParameters != NULL) {
            ...
            errorCode = uPortTaskCreate(posGetTask, "gnssPosCallback",
...
int32_t uGnssPosGetStreamedStart(
    uDeviceHandle_t gnssHandle,
    int32_t rateMs,
    void (*pCallback)(uDeviceHandle_t gnssHandle, int32_t errorCode, int32_t latitudeX1e7, int32_t longitudeX1e7,
                      int32_t altitudeMillimetres, int32_t radiusMillimetres, int32_t speedMillimetresPerSecond, int32_t svs,
  ...
  uGnssPrivateMessageId_t ubxNavPvtMessageId = {
        ...
        if (pInstance->pStreamedPosition == NULL) {
          ...
          if (pStreamedPosition != NULL) {
            ...
            if (rateMs >= 0) {
              ...
              if (uGnssPrivateGetRate(pInstance, &measurementPeriodMs, &navigationCount,
              NULL) != rateMs) {
                // Set the measurement rate, with a navigation count of 1
                // and leaving the time system unchanged
                errorCode = uGnssPrivateSetRate(pInstance, rateMs, 1, U_GNSS_TIME_SYSTEM_NONE);
                if (errorCode == 0) {
            ...
            if (errorCode == 0) {
              ...
              if (U_GNSS_PRIVATE_HAS(pInstance->pModule, U_GNSS_PRIVATE_FEATURE_OLD_CFG_API)) {
                messageRate = uGnssPrivateGetMsgRate(pInstance, &ubxNavPvtMessageId);
                if (messageRate != 1) {
                  errorCode = uGnssPrivateSetMsgRate(pInstance, &ubxNavPvtMessageId, 1);
                  if (errorCode == 0) {
                    pStreamedPosition->messageRate = messageRate;
                  }
                }
              } else {
                if (uGnssCfgPrivateValGetListAlloc(pInstance, &keyId, 1, &pCfgVal, U_GNSS_CFG_VAL_LAYER_RAM) == 1) {
                  messageRate = (int32_t) pCfgVal->value;
                  uPortFree(pCfgVal);
                }
                if (messageRate != (int32_t) cfgVal.value) {
                  errorCode = uGnssCfgPrivateValSetList(pInstance, &cfgVal, 1, U_GNSS_CFG_VAL_TRANSACTION_NONE,
            ...
            if (errorCode == 0) {
              ...
              errorCode = uGnssMsgPrivateReceiveStart(pInstance, &ubxNavPvtMessageId, messageCallback, pInstance);
              if (errorCode >= 0) {
                // And we're off
...
int32_t uGnssPosSetRrlpMode(uDeviceHandle_t gnssHandle, uGnssRrlpMode_t mode) {
  ...
  if (gUGnssPrivateMutex != NULL) {
    ...
    errorCode = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
    pInstance = pUGnssPrivateGetInstance(gnssHandle);
    if ((pInstance != NULL) && (mode < sizeof(gRrlpModeToUbxRxmMessageClass) / sizeof(gRrlpModeToUbxRxmMessageClass[0]))) {
      errorCode = (int32_t) U_ERROR_COMMON_NOT_SUPPORTED;
      if ((mode == U_GNSS_RRLP_MODE_MEASX) || U_GNSS_PRIVATE_HAS(pInstance->pModule, U_GNSS_PRIVATE_FEATURE_RXM_MEAS_50_20_C12_D12)) {
...
int32_t uGnssPosGetRrlp(uDeviceHandle_t gnssHandle, char *pBuffer, size_t sizeBytes, int32_t svsThreshold, int32_t cNoThreshold,
                        int32_t multipathIndexLimit, int32_t pseudorangeRmsErrorIndexLimit,
                        bool (*pKeepGoingCallback)(uDeviceHandle_t)) {
  ...
  if (gUGnssPrivateMutex != NULL) {
    ...
    pInstance = pUGnssPrivateGetInstance(gnssHandle);
    if ((pInstance != NULL) && (pBufferUint8 != NULL) && (sizeBytes >= U_UBX_PROTOCOL_OVERHEAD_LENGTH_BYTES)) {
      ...
      while ((errorCodeOrLength == (int32_t) U_ERROR_COMMON_TIMEOUT)
        ...
        numBytes = uGnssPrivateSendReceiveUbxMessage(pInstance, 0x02, messageClass, NULL, 0,
        ...
        if ((numBytes > 0) && (pInstance->rrlpMode == U_GNSS_RRLP_MODE_MEASX)) {
          ...
          if ((((svsThreshold >= 0) || (cNoThreshold >= 0) || (multipathIndexLimit >= 0) || (pseudorangeRmsErrorIndexLimit >= 0))
              && (numBytes >= 34))) {
            ...
            if ((numBytes > 0) && ((cNoThreshold >= 0) || (multipathIndexLimit >= 0) || (pseudorangeRmsErrorIndexLimit >= 0))) {
