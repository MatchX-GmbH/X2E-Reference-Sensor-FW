 ...
 */
U_WEAK
int32_t uDeviceCallback(const char *pOperationType, void *pOperationParam1, void *pOperationParam2) {
  (void) pOperationType;
...
int32_t uDeviceInit() {
  ...
  if (errorCode == 0) {
    ...
    if ((errorCode == (int32_t) U_ERROR_COMMON_NOT_IMPLEMENTED) || (errorCode == (int32_t) U_ERROR_COMMON_NOT_SUPPORTED)) {
      errorCode = 0;
    }
  }
  if (errorCode == 0) {
...
int32_t uDeviceGetDefaults(uDeviceType_t deviceType, uDeviceCfg_t *pDeviceCfg) {
...
int32_t uDeviceClose(uDeviceHandle_t devHandle, bool powerOff) {
  ...
  if (errorCode == 0) {
    ...
    if (errorCode == 0) {
      errorCode = uDeviceCallback("close", (void*) (U_DEVICE_INSTANCE(devHandle)->deviceType), (void*) powerOff);
    }
