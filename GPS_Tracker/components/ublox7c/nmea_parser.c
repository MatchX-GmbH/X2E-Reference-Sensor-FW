    ...
    /* End of statement */
      ...
      if (esp_gps->crc == crc) {
        ...
        if (((esp_gps->parsed_statement) & esp_gps->all_statements) == esp_gps->all_statements) {
          ...
          esp_event_post_to(esp_gps->event_loop_hdl, ESP_NMEA_EVENT, GPS_UPDATE, &(esp_gps->parent), sizeof(gps_t),
...
//      ub7_init(&gps, config->uart.uart_port, UB7_BAUD_9600, config->uart.tx_pin, config->uart.rx_pin, config->uart.en_pin));
  ...
  ESP_ERROR_CHECK(ub7_set_message(&gps, UB7_MSG_GSV, CONFIG_NMEA_STATEMENT_GSV));
  ESP_ERROR_CHECK(ub7_set_message(&gps, UB7_MSG_RMC, CONFIG_NMEA_STATEMENT_RMC));
  ESP_ERROR_CHECK(ub7_set_message(&gps, UB7_MSG_GLL, CONFIG_NMEA_STATEMENT_GLL));
  ESP_ERROR_CHECK(ub7_set_message(&gps, UB7_MSG_VTG, CONFIG_NMEA_STATEMENT_VTG));
  ESP_ERROR_CHECK(ub7_set_message(&gps, UB7_MSG_DTM, false));
  ESP_ERROR_CHECK(ub7_set_message(&gps, UB7_MSG_GBS, false));
...
//  uart_driver_delete(config->uart.uart_port);
  ...
  return esp_gps;
  /*Error Handling*/
  err_task_create: esp_event_loop_delete(esp_gps->event_loop_hdl);
  err_eloop: err_uart_install: uart_driver_delete(esp_gps->uart_port);
  err_uart_config: err_buffer: free(esp_gps->buffer);
  err_gps: free(esp_gps);
  return NULL;
}
