...
/** \file
* MfNtag42XDna specific AL-Component of Reader Library Framework.
* $Author: NXP $
* $Revision: $ (v07.09.00)
* $Date: $
*
    ...
    uint8_t *pPcdCapsOut, uint8_t *pPdCapsOut)
{
    ...
    if ((pDataParams->pKeyStoreDataParams == NULL) ||
        (pDataParams->pCryptoDataParamsEnc == NULL) ||
        (pDataParams->pCryptoDataParamsMac == NULL) ||
        (pDataParams->pCryptoRngDataParams == NULL))
    {
        return PH_ADD_COMPCODE(PH_ERR_USE_CONDITION, PH_COMP_AL_MFNTAG42XDNA);
    }
    ...
    uint8_t bFileOption, uint8_t *pAccessRights, uint8_t bAdditionalInfoLen, uint8_t * bAdditionalInfo)
{
    ...
    (void) bAdditionalInfoLen;
    (void) bAdditionalInfo;
    ...
    /* Check if Parameter passed as per Ref arch or as per MIFARE DESFire EV2 contactless IC. */
    ...
    {
        /* In this Case, here the bFileOption parameter is prepared as per MIFARE DESFire EV2 Convention.
