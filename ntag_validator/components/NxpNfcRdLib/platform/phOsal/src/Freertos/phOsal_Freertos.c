...
{
#ifdef PH_OSAL_ENABLE_TRUSTZONE
   portALLOCATE_SECURE_CONTEXT(stackSizeInNum);
   return PH_OSAL_SUCCESS;
#else
   return PH_OSAL_ADD_COMPCODE(PH_OSAL_UNSUPPORTED_COMMAND, PH_COMP_OSAL);
#endif
}
...
phStatus_t phOsal_TimerCreate(phOsal_Timer_t *timerHandle, pphOsal_TimerObj_t timerObj)
{
    ...
    *timerHandle = xTimerCreateStatic(((const char *)timerObj->pTimerName), TimerPeriod, timerObj->opt, timerObj->arg,
       (TimerCallbackFunction_t)timerObj->timerCb, &sTimerBuffer[dwFreeIndex]);
