...
// Check a blank tag
//==========================================================================
static int8_t CheckBlankTag(void) {
  if (AuthWithKey(0, true) < 0) {
    return -1;
  }
  if (AuthWithKey(1, true) < 0) {
    return -1;
  }
  if (AuthWithKey(2, true) < 0) {
    return -1;
  }
  if (AuthWithKey(3, true) < 0) {
    return -1;
    ...
    if (poll_ret == RET_POLL_ERROR) {
      ...
      if (CheckNtag() < 0) {
        ...
        switch (gAction) {
          ...
          break;
          case NTAG_ACTION_CHK_BLANK:
          if (CheckBlankTag() < 0) {
            PrintLine("INFO. Not a blank tag.");
          }
          else {
            PrintLine("INFO. A blank tag.");
            gActionResult.actionSuccess = true;
          }
          break;
        }
