...
static void populateContext(uDeviceGnssInstance_t *pContext, uGnssTransportHandle_t gnssTransportHandle,
...
static int32_t addDevice(uGnssTransportHandle_t gnssTransportHandle, uDeviceTransportType_t deviceTransportType,
                         const uDeviceCfgGnss_t *pCfgGnss, uDeviceHandle_t *pDeviceHandle) {
  ...
  if ((deviceTransportType >= 0)
      && (deviceTransportType < sizeof(gDeviceToGnssTransportType) / sizeof(gDeviceToGnssTransportType[0]))) {
  ...
  if (pContext != NULL) {
    ...
    errorCode = uGnssAdd((uGnssModuleType_t) pCfgGnss->moduleType, gnssTransportType, gnssTransportHandle, pCfgGnss->pinEnablePower,
    false,
                         pDeviceHandle);
    if (errorCode == 0) {
...
int32_t uDevicePrivateGnssAdd(const uDeviceCfg_t *pDevCfg, uDeviceHandle_t *pDeviceHandle) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
    ...
    if (pCfgGnss->version == 0) {
      switch (pDevCfg->transportType) {
        ...
        case U_DEVICE_TRANSPORT_TYPE_UART_2:
          ...
          errorCode = uPortUartOpen(pCfgUart->uart, pCfgUart->baudRate, NULL,
          U_GNSS_UART_BUFFER_LENGTH_BYTES,
                                    pCfgUart->pinTxd, pCfgUart->pinRxd, pCfgUart->pinCts, pCfgUart->pinRts);
          if (errorCode >= 0) {
            gnssTransportHandle.uart = errorCode;
            errorCode = addDevice(gnssTransportHandle, pDevCfg->transportType, pCfgGnss, pDeviceHandle);
            if (errorCode < 0) {
              // Clean up on error
        ...
        case U_DEVICE_TRANSPORT_TYPE_SPI:
          ...
          // Open SPI.
          errorCode = uPortSpiOpen(pCfgSpi->spi, pCfgSpi->pinMosi, pCfgSpi->pinMiso, pCfgSpi->pinClk,
          true);
          if (errorCode >= 0) {
            gnssTransportHandle.spi = errorCode;
            errorCode = uPortSpiControllerSetDevice(errorCode, &(pCfgSpi->device));
            if (errorCode == 0) {
              errorCode = addDevice(gnssTransportHandle, pDevCfg->transportType, pCfgGnss, pDeviceHandle);
            }
            if (errorCode < 0) {
