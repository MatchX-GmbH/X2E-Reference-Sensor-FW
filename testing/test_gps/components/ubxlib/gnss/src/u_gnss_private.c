...
const uGnssPrivateModule_t gUGnssPrivateModuleList[] = {
    ...
    },
    {
        ...
        ((1UL << (int32_t) U_GNSS_PRIVATE_FEATURE_CFGVALXXX) | (1UL << (int32_t) U_GNSS_PRIVATE_FEATURE_OLD_CFG_API)
            | (1UL << (int32_t) U_GNSS_PRIVATE_FEATURE_GEOFENCE) /* features */
        ) },
    {
        U_GNSS_MODULE_TYPE_M10,
        ((1UL << (int32_t) U_GNSS_PRIVATE_FEATURE_CFGVALXXX) | (1UL << (int32_t) U_GNSS_PRIVATE_FEATURE_RXM_MEAS_50_20_C12_D12) /* features */
        ) },
    {
        U_GNSS_MODULE_TYPE_M7,
        (1UL << (int32_t) U_GNSS_PRIVATE_FEATURE_OLD_CFG_API) /* features */
    } };
 ...
 */
const size_t gUGnssPrivateModuleListSize = sizeof(gUGnssPrivateModuleList) / sizeof(gUGnssPrivateModuleList[0]);
  ...
  if (pNmeaIdWanted != NULL) {  // A wanted string of NULL matches anything
    ...
    if (pNmeaIdActual != NULL) {
      ...
      while ((*pNmeaIdWanted != 0) && match) {
        if ((*pNmeaIdActual == 0) || ((*pNmeaIdWanted != '?') && (*pNmeaIdWanted != *pNmeaIdActual))) {
          match = false;
        }
        pNmeaIdActual++;
        pNmeaIdWanted++;
...
static bool ubxIdMatch(uint16_t ubxIdActual, uint16_t ubxIdWanted) {
  ...
  if ((ubxIdWanted & (U_GNSS_UBX_MESSAGE_CLASS_ALL << 8)) == (U_GNSS_UBX_MESSAGE_CLASS_ALL << 8)) {
...
static int32_t streamGetFromRingBuffer(uGnssPrivateInstance_t *pInstance, int32_t readHandle, char *pBuffer, size_t size,
                                       size_t offset, int32_t maxTimeMs,
                                       bool andRemove) {
  ...
  if (pInstance != NULL) {
    ...
    while ((leftToRead > 0) && (uPortGetTickTimeMs() - startTimeMs < maxTimeMs)) {
      if (andRemove) {
        receiveSize = (int32_t) uRingBufferReadHandle(&(pInstance->ringBuffer), readHandle, pBuffer, leftToRead);
      } else {
        receiveSize = (int32_t) uRingBufferPeekHandle(&(pInstance->ringBuffer), readHandle, pBuffer, leftToRead, offset);
        offset += receiveSize;
      }
...
static int32_t sendMessageStream(uGnssPrivateInstance_t *pInstance, const char *pMessage, size_t messageLengthBytes, bool printIt) {
  ...
  switch (privateStreamTypeOrError) {
    case U_GNSS_PRIVATE_STREAM_TYPE_UART: {
      errorCodeOrSentLength = uPortUartWrite(pInstance->transportHandle.uart, pMessage, messageLengthBytes);
    }
      break;
    case U_GNSS_PRIVATE_STREAM_TYPE_I2C: {
      errorCodeOrSentLength = uPortI2cControllerSend(pInstance->transportHandle.i2c, pInstance->i2cAddress, pMessage,
                                                     messageLengthBytes, false);
    ...
    case U_GNSS_PRIVATE_STREAM_TYPE_SPI: {
      ...
      for (size_t x = 0; (offset < messageLengthBytes) && (errorCodeOrSentLength >= 0); x++) {
        ...
        errorCodeOrSentLength = uPortSpiControllerSendReceiveBlock(pInstance->transportHandle.spi, pMessage + offset, thisLength,
                                                                   spiBuffer, thisLength);
        if (errorCodeOrSentLength > 0) {
    ...
    case U_GNSS_PRIVATE_STREAM_TYPE_VIRTUAL_SERIAL: {
      ...
      if (pDeviceSerial != NULL) {
        errorCodeOrSentLength = pDeviceSerial->write(pDeviceSerial, pMessage, messageLengthBytes);
      }
    }
      break;
    default:
...
// will be returned.
static int32_t receiveUbxMessageStream(uGnssPrivateInstance_t *pInstance, uGnssPrivateUbxReceiveMessage_t *pResponse,
  ...
  if ((pInstance != NULL) && (pResponse != NULL) && (pResponse->ppBody != NULL)) {
    ...
    errorCodeOrLength = uGnssPrivateReceiveStreamMessage(pInstance, &privateMessageId, pInstance->ringBufferReadHandlePrivate,
                                                         &pBuffer, 0, timeoutMs, NULL);
    if (errorCodeOrLength >= U_UBX_PROTOCOL_OVERHEAD_LENGTH_BYTES) {
      ...
      if (*(pResponse->ppBody) != NULL) {
        ...
        if (printIt) {
          uPortLog("U_GNSS: decoded UBX response 0x%02x 0x%02x", privateMessageId.id.ubx >> 8, privateMessageId.id.ubx & 0xff);
...
static int32_t sendReceiveUbxMessageAt(const uAtClientHandle_t atHandle, const char *pSend, size_t sendLengthBytes,
                                       uGnssPrivateUbxReceiveMessage_t *pResponse, int32_t timeoutMs,
                                       bool printIt) {
  ...
  if (pBuffer != NULL) {
    ...
    *(pBuffer + bytesToSend) = 0;
    ...
    if ((uAtClientUnlock(atHandle) == 0) && (bytesRead >= 0) && (pResponse->ppBody != NULL)) {
      ...
      if (x > 0) {
        ...
        if (captureSize > 0) {
          ...
          if ((uUbxProtocolDecode(pBuffer, x, &clsNack, &idNack, ackBody, sizeof(ackBody),
          NULL) == 2) && (ackBody[0] == pResponse->cls) && (ackBody[1] == pResponse->id)) {
            ...
            errorCodeOrLength = uUbxProtocolDecode(pBuffer, x, &(pResponse->cls), &(pResponse->id), *(pResponse->ppBody),
                                                   captureSize, NULL);
        ...
        if (printIt) {
          if (errorCodeOrLength >= 0) {
            uPortLog("U_GNSS: decoded UBX response 0x%02x 0x%02x", pResponse->cls, pResponse->id);
            if (errorCodeOrLength > 0) {
              uPortLog(":");
              uGnssPrivatePrintBuffer(*(pResponse->ppBody), errorCodeOrLength);
            }
...
// the response.
static int32_t sendReceiveUbxMessage(uGnssPrivateInstance_t *pInstance, int32_t messageClass, int32_t messageId,
  ...
  if ((pInstance != NULL) && (((pMessageBody == NULL) && (messageBodyLengthBytes == 0)) || (messageBodyLengthBytes > 0))
    ...
    if (pBuffer != NULL) {
      ...
      bytesToSend = uUbxProtocolEncode(messageClass, messageId, pMessageBody, messageBodyLengthBytes, pBuffer);
      if (bytesToSend > 0) {

        U_PORT_MUTEX_LOCK(pInstance->transportMutex);

        privateStreamTypeOrError = uGnssPrivateGetStreamType(pInstance->transportType);
        if ((pResponse != NULL) && (pResponse->ppBody != NULL) && (privateStreamTypeOrError >= 0)) {
          ...
          uRingBufferLockReadHandle(&(pInstance->ringBuffer), pInstance->ringBufferReadHandlePrivate);
          uRingBufferFlushHandle(&(pInstance->ringBuffer), pInstance->ringBufferReadHandlePrivate);
        }

        if (privateStreamTypeOrError >= 0) {
          errorCodeOrResponseLength = sendMessageStream(pInstance, pBuffer, bytesToSend, pInstance->printUbxMessages);
          if (errorCodeOrResponseLength >= 0) {
            errorCodeOrResponseLength = receiveUbxMessageStream(pInstance, pResponse, pInstance->timeoutMs,
                                                                pInstance->printUbxMessages);
          }
        } else {
          // Not a stream, we're on AT
          //lint -e{1773} Suppress attempt to cast away const: I'm not!
          errorCodeOrResponseLength = sendReceiveUbxMessageAt((const uAtClientHandle_t) pInstance->transportHandle.pAt, pBuffer,
                                                              bytesToSend, pResponse, pInstance->timeoutMs,
                                                              pInstance->printUbxMessages);
        }
...
int32_t getRateUbxCfgRate(uGnssPrivateInstance_t *pInstance, int32_t *pMeasurementPeriodMs, int32_t *pNavigationCount,
  ...
  if (uGnssPrivateSendReceiveUbxMessage(pInstance, 0x06, 0x08,
  NULL,
                                        0, message, sizeof(message)) == sizeof(message)) {
    ...
    if (pTimeSystem != NULL) {
      ...
      *pTimeSystem = (uGnssTimeSystem_t) uUbxProtocolUint16Decode((const char*) &(message[4]));  // *NOPAD*;
    }
  }
...
int32_t setRateUbxCfgRate(uGnssPrivateInstance_t *pInstance, int32_t measurementPeriodMs, int32_t navigationCount,
  ...
  if ((measurementPeriodMs < 0) || (navigationCount < 0) || (timeSystem < 0)) {
    // Need to read the settings first as it is not possible
    // to leave any settings out in the UBX-CFG-RATE message
    errorCode = getRateUbxCfgRate(pInstance, &currentMeasurementPeriodMs, &currentNavigationCount, &currentTimeSystem);
  }
  ...
  if (errorCode >= 0) {
    ...
    *((uint16_t*) &(message[4])) = uUbxProtocolUint16Encode((uint16_t) timeSystem);  // *NOPAD*;
    // Send UBX-CFG-RATE
    errorCode = uGnssPrivateSendUbxMessage(pInstance, 0x06, 0x08, message, sizeof(message));
  }
...
int32_t getRateUbxCfgVal(uGnssPrivateInstance_t *pInstance, int32_t *pMeasurementPeriodMs, int32_t *pNavigationCount,
                         uGnssTimeSystem_t *pTimeSystem) {
  int32_t errorCode;
  uGnssCfgVal_t *pCfgValList = NULL;
  uGnssCfgVal_t *pTmp;
  uint32_t keyId = U_GNSS_CFG_VAL_KEY(U_GNSS_CFG_VAL_KEY_GROUP_ID_RATE, U_GNSS_CFG_VAL_KEY_ITEM_ID_ALL, 0);
  uint16_t items[sizeof(gCfgValKeyIdCfgRate) / sizeof(gCfgValKeyIdCfgRate[0])] = {
      0 };
  size_t itemsFound = 0;

  // Request all the rate items
  errorCode = uGnssCfgPrivateValGetListAlloc(pInstance, &keyId, 1, &pCfgValList, U_GNSS_CFG_VAL_LAYER_RAM);
  if ((errorCode > 0) && (pCfgValList != NULL)) {
    ...
    for (int32_t x = 0; x < errorCode; x++) {
      ...
      for (size_t y = 0; y < sizeof(gCfgValKeyIdCfgRate) / sizeof(gCfgValKeyIdCfgRate[0]); y++) {
...
int32_t setRateUbxCfgVal(uGnssPrivateInstance_t *pInstance, int32_t measurementPeriodMs, int32_t navigationCount,
                         uGnssTimeSystem_t timeSystem) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_SUCCESS;
  ...
  if (numEntries > 0) {
    ...
    errorCode = uGnssCfgPrivateValSetList(pInstance, val, numEntries, U_GNSS_CFG_VAL_TRANSACTION_NONE, U_GNSS_CFG_VAL_LAYER_RAM);
  }
...
static int32_t setProtocolOutUbxCfgPrtInner(uGnssPrivateInstance_t *pInstance, uGnssProtocol_t protocol,
bool onNotOff) {
  ...
  if (uGnssPrivateSendReceiveUbxMessage(pInstance, 0x06, 0x00, message, 1, message, sizeof(message)) == sizeof(message)) {
    ...
    *((uint16_t*) &(message[14])) = uUbxProtocolUint16Encode(mask);  // *NOPAD*
    ...
    errorCode = uGnssPrivateSendReceiveUbxMessage(pInstance, 0x06, 0x00, message, sizeof(message),
    NULL,
                                                  0);
...
static int32_t setProtocolOutUbxCfgPrt(uGnssPrivateInstance_t *pInstance, uGnssProtocol_t protocol,
bool onNotOff) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_PLATFORM;
  ...
  if (pInstance->transportType == U_GNSS_TRANSPORT_VIRTUAL_SERIAL) {
    ...
    if (uGnssPrivateSendReceiveUbxMessage(pInstance, 0x0a, 0x06,
    NULL,
                                          0, message, sizeof(message)) == sizeof(message)) {
...
static int32_t setProtocolOutUbxCfgVal(uGnssPrivateInstance_t *pInstance, uGnssProtocol_t protocol,
bool onNotOff) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_PLATFORM;
  uGnssCfgVal_t val[sizeof(gProtocolTypeToCfgValItemIdOutProt) / sizeof(gProtocolTypeToCfgValItemIdOutProt[0])];
  size_t numEntries = 0;

  if (pInstance->portNumber < sizeof(gPortToCfgValGroupIdOutProt) / sizeof(gPortToCfgValGroupIdOutProt[0])) {
    // Add the key/value pairs
    if (protocol == U_GNSS_PROTOCOL_ALL) {
      for (size_t x = 0; (x < sizeof(val) / sizeof(val[0])); x++) {
        ...
        val[x].keyId = U_GNSS_CFG_VAL_KEY(gPortToCfgValGroupIdOutProt[pInstance->portNumber], gProtocolTypeToCfgValItemIdOutProt[x],
      ...
      if (protocol < (sizeof(gProtocolTypeToCfgValItemIdOutProt) / sizeof(gProtocolTypeToCfgValItemIdOutProt[0]))) {
        val[0].keyId = U_GNSS_CFG_VAL_KEY(gPortToCfgValGroupIdOutProt[pInstance->portNumber],
                                          gProtocolTypeToCfgValItemIdOutProt[protocol], U_GNSS_CFG_VAL_KEY_SIZE_ONE_BIT);
...
static int32_t getProtocolOutUbxCfgVal(uGnssPrivateInstance_t *pInstance) {
  ...
  if (pInstance->portNumber < sizeof(gPortToCfgValGroupIdOutProt) / sizeof(gPortToCfgValGroupIdOutProt[0])) {
    ...
    keyId = U_GNSS_CFG_VAL_KEY(gPortToCfgValGroupIdOutProt[pInstance->portNumber], U_GNSS_CFG_VAL_KEY_ITEM_ID_ALL, 0);
    errorCodeOrBitMap = uGnssCfgPrivateValGetListAlloc(pInstance, &keyId, 1, &pCfgValList, U_GNSS_CFG_VAL_LAYER_RAM);
    if ((errorCodeOrBitMap > 0) && (pCfgValList != NULL)) {
      ...
      for (int32_t x = 0; x < numValues; x++) {
        // Go through the list looking for our key IDs
        for (size_t y = 0; y < sizeof(gProtocolTypeToCfgValItemIdOutProt) / sizeof(gProtocolTypeToCfgValItemIdOutProt[0]); y++) {
          if (pTmp->keyId
              == U_GNSS_CFG_VAL_KEY(gPortToCfgValGroupIdOutProt[pInstance->portNumber], gProtocolTypeToCfgValItemIdOutProt[y],
                                    U_GNSS_CFG_VAL_KEY_SIZE_ONE_BIT)) {
...
uGnssPrivateInstance_t* pUGnssPrivateGetInstance(uDeviceHandle_t handle) {
  ...
  uDeviceHandle_t gnssHandle = uNetworkGetDeviceHandle(handle, U_NETWORK_TYPE_GNSS);
...
int32_t uGnssPrivateGetRate(uGnssPrivateInstance_t *pInstance, int32_t *pMeasurementPeriodMs, int32_t *pNavigationCount,
                            uGnssTimeSystem_t *pTimeSystem) {
  ...
  if (pInstance != NULL) {
    if (U_GNSS_PRIVATE_HAS(pInstance->pModule, U_GNSS_PRIVATE_FEATURE_CFGVALXXX)) {
      errorCodeOrRate = getRateUbxCfgVal(pInstance, pMeasurementPeriodMs, pNavigationCount, pTimeSystem);
    } else {
      errorCodeOrRate = getRateUbxCfgRate(pInstance, pMeasurementPeriodMs, pNavigationCount, pTimeSystem);
    }
  }
...
int32_t uGnssPrivateSetRate(uGnssPrivateInstance_t *pInstance, int32_t measurementPeriodMs, int32_t navigationCount,
                            uGnssTimeSystem_t timeSystem) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;

  if (pInstance != NULL) {
    if (U_GNSS_PRIVATE_HAS(pInstance->pModule, U_GNSS_PRIVATE_FEATURE_CFGVALXXX)) {
      errorCode = setRateUbxCfgVal(pInstance, measurementPeriodMs, navigationCount, timeSystem);
    } else {
      errorCode = setRateUbxCfgRate(pInstance, measurementPeriodMs, navigationCount, timeSystem);
    }
  }
...
int32_t uGnssPrivateGetMsgRate(uGnssPrivateInstance_t *pInstance, uGnssPrivateMessageId_t *pPrivateMessageId) {
  int32_t errorCodeOrRate = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
  ...
  if ((pInstance != NULL) && (pPrivateMessageId != NULL)) {
    ...
    if ((pPrivateMessageId->type == U_GNSS_PROTOCOL_UBX)
        && U_GNSS_PRIVATE_HAS(pInstance->pModule, U_GNSS_PRIVATE_FEATURE_OLD_CFG_API)) {
      ...
      if (pInstance->portNumber < sizeof(message) - 2) {
        ...
        if (uGnssPrivateSendReceiveUbxMessage(pInstance, 0x06, 0x01, message, 2, message, sizeof(message)) == sizeof(message)) {
          // If the message class and ID are correct in the response,
          // the rate at the offset that is the port number
          if ((message[0] == messageClass) && (message[1] == messageId)) {
...
int32_t uGnssPrivateSetMsgRate(uGnssPrivateInstance_t *pInstance, uGnssPrivateMessageId_t *pPrivateMessageId, int32_t rate) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
  ...
  if ((pInstance != NULL) && (pPrivateMessageId != NULL)) {
    ...
    if ((pPrivateMessageId->type == U_GNSS_PROTOCOL_UBX)
      ...
      if (pInstance->portNumber < sizeof(message) - 2) {
        ...
        // Send UBX-CFG-MSG
        errorCode = uGnssPrivateSendUbxMessage(pInstance, 0x06, 0x01, message, sizeof(message));
      }
    }
  }
...
int32_t uGnssPrivateSetProtocolOut(uGnssPrivateInstance_t *pInstance, uGnssProtocol_t protocol,
bool onNotOff) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;

  if ((pInstance != NULL) && (pInstance->transportType != U_GNSS_TRANSPORT_AT)
      && (onNotOff || ((protocol != U_GNSS_PROTOCOL_ALL) && (protocol != U_GNSS_PROTOCOL_UBX)))) {
...
void uGnssPrivateCleanUpStreamedPos(uGnssPrivateInstance_t *pInstance) {
  ...
  uGnssCfgVal_t cfgVal = {
      .keyId = U_GNSS_CFG_VAL_KEY_ID_MSGOUT_UBX_NAV_PVT_I2C_U1 };
  ...
  if ((pInstance != NULL) && (pInstance->pStreamedPosition != NULL)) {
    ...
    if (pStreamedPosition->asyncHandle >= 0) {
      uGnssMsgPrivateReceiveStop(pInstance, pStreamedPosition->asyncHandle);
    }
    // Put any saved settings back
    if ((pStreamedPosition->measurementPeriodMs >= 0) || (pStreamedPosition->navigationCount >= 0)) {
      ...
      for (size_t x = 0; (x < tries) && (y < 0); x++) {
        y = uGnssPrivateSetRate(pInstance, pStreamedPosition->measurementPeriodMs, pStreamedPosition->navigationCount,
    ...
    if (pStreamedPosition->messageRate >= 0) {
      ...
      for (size_t x = 0; (x < tries) && (y < 0); x++) {
        if (U_GNSS_PRIVATE_HAS(pInstance->pModule, U_GNSS_PRIVATE_FEATURE_OLD_CFG_API)) {
          y = uGnssPrivateSetMsgRate(pInstance, &privateMessageId, pStreamedPosition->messageRate);
        } else {
          cfgVal.value = pStreamedPosition->messageRate;
          y = uGnssCfgPrivateValSetList(pInstance, &cfgVal, 1, U_GNSS_CFG_VAL_TRANSACTION_NONE, U_GNSS_CFG_VAL_LAYER_RAM);
        }
      }
    }
...
bool uGnssPrivateIsInsideCell(const uGnssPrivateInstance_t *pInstance) {
  ...
  if (pInstance != NULL) {
    ...
    if (pInstance->transportType == U_GNSS_TRANSPORT_AT) {
      ...
      bytesRead = uAtClientReadBytes(atHandle, buffer, sizeof(buffer) - 1, false);
      uAtClientResponseStop(atHandle);
...
void uGnssPrivateStopMsgReceive(uGnssPrivateInstance_t *pInstance) {
  ...
  if ((pInstance != NULL) && (pInstance->pMsgReceive != NULL)) {
    ...
    uRingBufferGiveReadHandle(&(pInstance->ringBuffer), pMsgReceive->ringBufferReadHandle);
...
int32_t uGnssPrivateMessageIdToPrivate(const uGnssMessageId_t *pMessageId, uGnssPrivateMessageId_t *pPrivateMessageId) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
    ...
    switch (pMessageId->type) {
      ...
      case U_GNSS_PROTOCOL_NMEA:
        ...
        if (pMessageId->id.pNmea != NULL) {
          strncpy(pPrivateMessageId->id.nmea, pMessageId->id.pNmea, sizeof(pPrivateMessageId->id.nmea));
        }
        errorCode = (int32_t) U_ERROR_COMMON_SUCCESS;
        break;
...
int32_t uGnssPrivateMessageIdToPublic(const uGnssPrivateMessageId_t *pPrivateMessageId, uGnssMessageId_t *pMessageId, char *pNmea) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
...
bool uGnssPrivateMessageIdIsWanted(uGnssPrivateMessageId_t *pMessageId, uGnssPrivateMessageId_t *pMessageIdWanted) {
  bool isWanted = false;

  if ((pMessageIdWanted->type == U_GNSS_PROTOCOL_ANY) || (pMessageIdWanted->type == U_GNSS_PROTOCOL_ALL)) {
    isWanted = true;
  } else if ((pMessageIdWanted->type == U_GNSS_PROTOCOL_UNKNOWN) && (pMessageId->type == U_GNSS_PROTOCOL_UNKNOWN)) {
    isWanted = true;
  } else if ((pMessageIdWanted->type == U_GNSS_PROTOCOL_RTCM) && (pMessageId->type == U_GNSS_PROTOCOL_RTCM)) {
    isWanted = rtcmIdMatch(pMessageId->id.rtcm, pMessageIdWanted->id.rtcm);
  } else if ((pMessageIdWanted->type == U_GNSS_PROTOCOL_NMEA) && (pMessageId->type == U_GNSS_PROTOCOL_NMEA)) {
    isWanted = nmeaIdMatch(pMessageId->id.nmea, pMessageIdWanted->id.nmea);
  } else if ((pMessageIdWanted->type == U_GNSS_PROTOCOL_UBX) && (pMessageId->type == U_GNSS_PROTOCOL_UBX)) {
...
int32_t uGnssPrivateGetStreamType(uGnssTransportType_t transportType) {
  ...
  if ((transportType >= 0)
      && (transportType < sizeof(gGnssPrivateTransportTypeToStream) / sizeof(gGnssPrivateTransportTypeToStream[0]))) {
  ...
  if (pInstance != NULL) {
    ...
    switch (privateStreamTypeOrError) {
      ...
      case U_GNSS_PRIVATE_STREAM_TYPE_I2C: {
        ...
        buffer[0] = 0xFD;
        errorCodeOrReceiveSize = uPortI2cControllerSend(pInstance->transportHandle.i2c, i2cAddress, buffer, 1, true);
        if (errorCodeOrReceiveSize == 0) {
          errorCodeOrReceiveSize = uPortI2cControllerSendReceive(pInstance->transportHandle.i2c, i2cAddress,
          NULL,
      ...
      case U_GNSS_PRIVATE_STREAM_TYPE_SPI: {
        ...
        NULL,
                                                                    0, spiBuffer, spiReadLength);
        if (errorCodeOrReceiveSize > 0) {
          // This will add any non-fill SPI received data to the
          // internal SPI ring buffer
          errorCodeOrReceiveSize = uGnssPrivateSpiAddReceivedData(pInstance, spiBuffer, errorCodeOrReceiveSize);
        }
      }
        break;
      case U_GNSS_PRIVATE_STREAM_TYPE_VIRTUAL_SERIAL: {
...
// uGnssPrivateStreamFillRingBuffer() which may be called at any time by
// the message receive task over in u_gnss_msg.c
int32_t uGnssPrivateStreamDecodeRingBuffer(uRingBuffer_t *pRingBuffer, int32_t readHandle,
    ...
    while (1) {
      ...
      if (errorCodeOrLength <= 0) {
        ...
        break;
                ...
                uPortLog(", %d byte(s)\n", errorCodeOrLength);
#endif
        if ((pPrivateMessageId->type == U_GNSS_PROTOCOL_UBX) && (msg.type == U_GNSS_PROTOCOL_UBX)
...
int32_t uGnssPrivateStreamFillRingBuffer(uGnssPrivateInstance_t *pInstance, int32_t timeoutMs, int32_t maxTimeMs) {
  int32_t errorCodeOrLength = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
  int32_t startTimeMs;
  int32_t privateStreamTypeOrError;
  int32_t receiveSize;
  ...
  if (pInstance != NULL) {
    ...
    if ((pInstance->pMsgReceive != NULL) && uPortTaskIsThis(pInstance->pMsgReceive->taskHandle)) {
    ...
    if (privateStreamTypeOrError >= 0) {
        ...
        if (receiveSize > 0) {
          ...
          switch (privateStreamTypeOrError) {
            ...
            case U_GNSS_PRIVATE_STREAM_TYPE_I2C:
              ...
              receiveSize = uPortI2cControllerSendReceive(pInstance->transportHandle.i2c, pInstance->i2cAddress,
              NULL,
                                                          0, pTemporaryBuffer, receiveSize);
              break;
            case U_GNSS_PRIVATE_STREAM_TYPE_SPI:
              ...
              receiveSize = (int32_t) uRingBufferRead(pInstance->pSpiRingBuffer, pTemporaryBuffer, receiveSize);
              break;
            case U_GNSS_PRIVATE_STREAM_TYPE_VIRTUAL_SERIAL: {
          ...
          if (receiveSize >= 0) {
            ...
            if (!uRingBufferForceAdd(&(pInstance->ringBuffer), pTemporaryBuffer, receiveSize)) {
              errorCodeOrLength = (int32_t) U_ERROR_COMMON_NO_MEMORY;
            }
          } else {
            // Error case
      ...
      } while (((errorCodeOrLength == (int32_t) U_ERROR_COMMON_TIMEOUT) || (errorCodeOrLength >= 0)) && (timeoutMs > 0)
          && (ringBufferAvailableSize > 0) &&
...
int32_t uGnssPrivateStreamReadRingBuffer(uGnssPrivateInstance_t *pInstance, int32_t readHandle, char *pBuffer, size_t size,
                                         int32_t maxTimeMs) {
  return streamGetFromRingBuffer(pInstance, readHandle, pBuffer, size, 0, maxTimeMs, true);
}

// Take a peek at the data in the internal ring buffer.
int32_t uGnssPrivateStreamPeekRingBuffer(uGnssPrivateInstance_t *pInstance, int32_t readHandle, char *pBuffer, size_t size,
                                         size_t offset, int32_t maxTimeMs) {
  return streamGetFromRingBuffer(pInstance, readHandle, pBuffer, size, offset, maxTimeMs, false);
}

// Send raw bytes over UART or I2C or SPI or virtual serial.
int32_t uGnssPrivateSendOnlyStreamRaw(uGnssPrivateInstance_t *pInstance, const char *pBuffer, size_t size) {
  int32_t errorCodeOrSentLength = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;

  if (pInstance != NULL) {
    if ((uGnssPrivateGetStreamType(pInstance->transportType) >= 0) && (pBuffer != NULL) && (size > 0)) {

      U_PORT_MUTEX_LOCK(pInstance->transportMutex);

      errorCodeOrSentLength = sendMessageStream(pInstance, pBuffer, size, pInstance->printUbxMessages);
...
int32_t uGnssPrivateSendOnlyStreamUbxMessage(uGnssPrivateInstance_t *pInstance, int32_t messageClass, int32_t messageId,
                                             const char *pMessageBody, size_t messageBodyLengthBytes) {
  int32_t errorCodeOrSentLength = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
  int32_t bytesToSend = 0;
  char *pBuffer;
  ...
  if (pInstance != NULL) {
    if ((uGnssPrivateGetStreamType(pInstance->transportType) >= 0)
        && (((pMessageBody == NULL) && (messageBodyLengthBytes == 0)) || (messageBodyLengthBytes > 0))) {
      ...
      if (pBuffer != NULL) {
        ...
        errorCodeOrSentLength = sendMessageStream(pInstance, pBuffer, bytesToSend, pInstance->printUbxMessages);
...
int32_t uGnssPrivateSendOnlyCheckStreamUbxMessage(uGnssPrivateInstance_t *pInstance, int32_t messageClass, int32_t messageId,
                                                  const char *pMessageBody, size_t messageBodyLengthBytes) {
  int32_t errorCodeOrLength = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
  ...
  if (pInstance != NULL) {
    if (uGnssPrivateGetStreamType(pInstance->transportType) >= 0) {
      if (pInstance->transportType == U_GNSS_TRANSPORT_VIRTUAL_SERIAL) {
        ...
        errorCodeOrLength = uGnssPrivateSendOnlyStreamUbxMessage(pInstance, messageClass, messageId, pMessageBody,
                                                                 messageBodyLengthBytes);
      } else {
        // Send UBX-MON-MSGPP to get the number of messages received
        errorCodeOrLength = uGnssPrivateSendReceiveUbxMessage(pInstance, 0x0a, 0x06,
        NULL,
                                                              0, message, sizeof(message));
...
int32_t uGnssPrivateReceiveStreamMessage(uGnssPrivateInstance_t *pInstance, uGnssPrivateMessageId_t *pPrivateMessageId,
                                         int32_t readHandle, char **ppBuffer, size_t size, int32_t timeoutMs,
  ...
  if ((pInstance != NULL) && (pPrivateMessageId != NULL) && (ppBuffer != NULL) && ((*ppBuffer == NULL) || (size > 0))) {
    errorCodeOrLength = (int32_t) U_ERROR_COMMON_TIMEOUT;
    startTimeMs = uPortGetTickTimeMs();
    ...
    do {
      ...
      ringBufferSize = uRingBufferDataSizeHandle(&(pInstance->ringBuffer), readHandle);
      if (ringBufferSize < 0) {
        errorCodeOrLength = ringBufferSize;
      } else if (ringBufferSize > 0) {
        // Deal with any discard from a previous run around this loop
        discardSize -= uRingBufferReadHandle(&(pInstance->ringBuffer), readHandle,
        NULL,
                                             discardSize);
        if (discardSize == 0) {
          // Attempt to decode a message/message header from the ring buffer
          errorCodeOrLength = uGnssPrivateStreamDecodeRingBuffer(&(pInstance->ringBuffer), readHandle, pPrivateMessageId);
          if (errorCodeOrLength > 0) {
            if (*ppBuffer == NULL) {
            ...
            if (*ppBuffer != NULL) {
              ...
              errorCodeOrLength = uGnssPrivateStreamReadRingBuffer(pInstance, readHandle, *ppBuffer, errorCodeOrLength, y);
            } else {
      ...
      // but always checking the guard time/callback.
        ...
        && (errorCodeOrLength != (int32_t) U_ERROR_COMMON_NO_MEMORY)) || (discardSize > 0)) && (timeoutMs > 0)
...
int32_t uGnssPrivateSpiAddReceivedData(uGnssPrivateInstance_t *pInstance, const char *pBuffer, size_t size) {
  int32_t errorCodeOrLength = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
  const char *pTmp = pBuffer;
  int32_t y;

  if ((pInstance != NULL) && (pInstance->pSpiRingBuffer != NULL) && (pBuffer != NULL) && (size > 0)) {
    ...
    if (pInstance->spiFillThreshold > 0) {
      ...
      uRingBufferFlushValue(pInstance->pSpiRingBuffer, U_GNSS_PRIVATE_SPI_FILL, pInstance->spiFillThreshold);
    }
...
int32_t uGnssPrivateSendReceiveUbxMessage(uGnssPrivateInstance_t *pInstance, int32_t messageClass, int32_t messageId,
                                          const char *pMessageBody, size_t messageBodyLengthBytes, char *pResponseBody,
  ...
  return sendReceiveUbxMessage(pInstance, messageClass, messageId, pMessageBody, messageBodyLengthBytes, &response);
}

// Send a UBX format message and receive a response of unknown length.
int32_t uGnssPrivateSendReceiveUbxMessageAlloc(uGnssPrivateInstance_t *pInstance, int32_t messageClass, int32_t messageId,
                                               const char *pMessageBody, size_t messageBodyLengthBytes, char **ppResponseBody) {
  uGnssPrivateUbxReceiveMessage_t response;
...
int32_t uGnssPrivateSendUbxMessage(uGnssPrivateInstance_t *pInstance, int32_t messageClass, int32_t messageId,
                                   const char *pMessageBody, size_t messageBodyLengthBytes) {
  int32_t errorCode = (int32_t) U_ERROR_COMMON_INVALID_PARAMETER;
  uGnssPrivateUbxReceiveMessage_t response;
  int32_t timeoutMs;
  ...
  if (pInstance != NULL) {
    ...
    startTimeMs = uPortGetTickTimeMs();
    errorCode = sendReceiveUbxMessage(pInstance, messageClass, messageId, pMessageBody, messageBodyLengthBytes, &response);
    ...
    // with the right body.
    while ((errorCode != 2) && (response.cls != 0x05) && (ackBody[0] != (char) messageClass) && (ackBody[1] != (char) messageId)
        && !((response.id == 0x01) || (response.id == 0x00)) && (uGnssPrivateGetStreamType(pInstance->transportType) >= 0)
        && (uPortGetTickTimeMs() - startTimeMs < timeoutMs)) {
      response.cls = 0x05;
      response.id = -1;
      errorCode = receiveUbxMessageStream(pInstance, &response, timeoutMs, pInstance->printUbxMessages);
    }
