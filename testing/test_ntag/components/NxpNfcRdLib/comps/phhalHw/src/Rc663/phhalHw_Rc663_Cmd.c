                                     ...
                                     )
{
    ...
    /* Perform command */
    wStatus = phhalHw_Rc663_Command_Int(
        pDataParams,
    ...
    {
        /* Note: These register values are arrived based on best performance for ISO/IEC15693 protocol for 65x65 antenna. */
        /* Update Driver Control Register. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(
            pDataParams,
            (uint8_t)PHHAL_HW_RC663_REG_DRVCON,
            (uint8_t)PROTOCOL_SLI_1_4_SSC26_DRVCON_REG_V));

        /* Update Rx-Threshold Register. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(
            pDataParams,
            (uint8_t)PHHAL_HW_RC663_REG_RXTHRESHOLD,
            (uint8_t)PROTOCOL_SLI_1_4_SSC26_RXTHRESHOLD_REG_V));

        /* Update Rx-Analog Register. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(
            pDataParams,
            (uint8_t)PHHAL_HW_RC663_REG_RXANA,
            (uint8_t)PROTOCOL_SLI_1_4_SSC26_RXANA_REG_V));
    }
    ...
    )
{
    ...
    {
        /* Note: These register values are arrived based on best performance for ISO/IEC15693 protocol for 65x65 antenna. */
        /* Update Calibration register of the receiver. */
        PH_CHECK_SUCCESS_FCT(statusTmp, phhalHw_Rc663_WriteRegister(
            pDataParams,
            (uint8_t)PHHAL_HW_RC663_REG_RXSVETTE,
            (uint8_t)PHHAL_HW_RC663_REG_RXSVETTE_V));
    }
                                       ...
                                       )
{
    ...
    // NFCCOMPON_PRINTLINE("phhalHw_Rc663_WriteRegister %02X=%02X", bAddress, bValue);
    /* RS232 protocol */
                                      ...
                                      )
{
    ...
    // NFCCOMPON_PRINTLINE("phhalHw_Rc663_ReadRegister %02X, value=%02X", bAddress, *pValue);
